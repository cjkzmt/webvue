...
export const onSubmit = async () => {
  ...
  if (data.code === '000000') {
    ElMessage.success(`${msgText.value}链接成功`)
    queryUrlText()
  } else {
    ElMessage.error(`${msgText.value}链接失败`)
    throw new Error(`${msgText.value}链接失败`)
  }
}
...
//动作
export const queryUrlText = async (params?: QueryCondition) => {
  Object.assign(queryCondition.value, params)
  const { data } = await getUrlTextPages(queryCondition.value)
  if (data.code === '000000') {
...
export const handleDelete = async (id: number) => {
  await ElMessageBox.confirm('此操作将永久删除该链接, 是否继续?', '提示', {
  ...
  const { data } = await deleteUrlText(id)
  if (data.code === '000000') {
