...
import { queriedResult, queryCondition, queryMusic, handleDelete, handleStatusChange } from '@/composables/useMusic'
import { timeFormatter } from '@/utils/timeHandler'
import { ref } from 'vue'
import DlgPCCreateOrEdit from './DlgVideoClipsCreateOrEdit.vue'
queryMusic()
...
<template>
  <el-card style="max-width: auto">
    <template #header>
      ...
      >
        <el-form :inline="true" :model="queryCondition" class="demo-form-inline">
          <!-- <el-form-item label="片段品牌">
            <el-input v-model="queryCondition.Brand" placeholder="请输入片段品牌" clearable />
          </el-form-item> -->
          ...
          <el-form-item>
            <el-button type="primary" @click="queryMusic({ currentPage: 1 })">查询</el-button>
          </el-form-item>
        </el-form>
        <el-button
          class="Createbutton"
    ...
    <el-table :data="queriedResult.records" border style="width: 100%">
      ...
      <el-table-column prop="name" label="片段名" width="180" align="center" />
      <el-table-column prop="clipsum" label="片段数量" width="180" align="center" />
      <el-table-column
        prop="createdTime"
        label="录入时间"
        width="180"
    ...
    <el-pagination
      ...
      @size-change="(pageSize: number) => queryMusic({ pageSize, currentPage: 1 })"
      @current-change="(currentPage: number) => queryMusic({ currentPage })"
    />
