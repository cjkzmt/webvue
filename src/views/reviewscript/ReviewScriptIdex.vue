...
import { queriedResult, queryCondition, queryScript, handleDelete ,AddTask ,RefreshTask,EnablesSriptStatus,handleDeleteCopy} from '@/composables/useScript'
// import { timeFormatter } from '@/utils/timeHandler'
import { ref } from 'vue'
import DlgScriptCreateOrEdit from './DlgReviewScriptCreateOrEdit.vue'
queryCondition.value.getinfo = 'ReviewScript'
queryScript()
...
<template>
  <el-card style="max-width: auto">
    ...
    <el-table :data="queriedResult.records" border style="width: 100%">
      ...
      <el-table-column label="删除脚本" align="center" width="90" v-slot="{ row }">
        <el-button type="danger" @click="handleDelete(row.id)">删除</el-button>
      </el-table-column>
      <el-table-column label="脚本初稿" align="center" width="auto">
        <template v-slot="{ row }">
          <div style="text-align: left;">{{ row.drafline }}</div>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="90" v-slot="{ row }">
       ...
       <el-table-column label="脚本文案" align="center" width="auto">
        <template v-slot="{ row }">
          <div style="text-align: left;">{{ row.line }}</div>
        </template>
      </el-table-column>
      <el-table-column label="通过" align="center" width="90" v-slot="{ row }">
        <el-button type="primary" @click="EnablesSriptStatus(row.id)">通过</el-button>
      </el-table-column>
      <el-table-column label="样本文案" align="center" width="auto">
        <template v-slot="{ row }">
          <div style="text-align: left;">{{ row.copytext }}</div>
        </template>
      </el-table-column>
      <el-table-column label="删除样本" align="center" width="90" v-slot="{ row }">
        <el-button type="danger" @click="handleDeleteCopy(row.copyId)">删除</el-button>
      </el-table-column>
      <!-- <el-table-column
        prop="publishtime"
        label="发布时间"
